...
A_Ab30_insert50=group(A_Ab30_insert,pas=50) #binning by 50bp
...
#stats
for bank in [A_Ab30_insert, B_Ab30_insert]:
    print "****"  
    print "inserts count =                    %i"%bank[1:,:].sum()
    print "ratio forward =                    %f"%(float(ban[1,:].sum())/bank[1:,:].sum())
    print "mean insertion =                   %f"%np.mean(bank[1:,:].sum(axis=0))
    print "variance =                         %f"%np.var(bank[1:,:].sum(axis=0))
print "****"
...
#stats
for bank in [A_Pa14_insert, B_Pa14_insert]:
    print "****"  
    print "inserts count =                    %i"%bank[1:,:].sum()
    print "ratio forward =                    %f"%(float(bank[1,:].sum())/bank[1:,:].sum())
    print "mean insertion =                   %f"%np.mean(bank[1:,:].sum(axis=0))
    print "variance =                         %f"%np.var(bank[1:,:].sum(axis=0))
    print "concentration closes to patterns = %f"%pourcentMotif(bank, "Pa14")
print "****"
...
#graph focus on beginning of bacteria
...
for yrs in YRS:
    ...
    if sens:
        for bank in [A_Pa14_insert, B_Pa14_insert]:
            table.append(np.max(bank[1:,i+49:i+81].sum(axis=0)))
            table.append(round(np.mean(bank[1:,i+49:i+81].sum(axis=0)),2))
            
    elif not sens:
        for bank in [A_Pa14_insert, B_Pa14_insert]:
            table.append(np.max(bank[1:,i-81:i-49].sum(axis=0)))
            table.append(round(np.mean(bank[1:,i-81:i-49].sum(axis=0)),2))
...
##March 2016
#some news graphs with graphMirror() function
